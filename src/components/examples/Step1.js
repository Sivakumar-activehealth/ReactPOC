...
isValidated() {
    ...
    if (Object.keys(validateNewInput).every((k) => { return validateNewInput[k] === true })) {
        if (this.props.getStore().personName != userInput.personName) { // only update store of something changed
  ...
  validationCheck() {
   ...
   _validateData(data) {
    return  {
      PersonVal: (data.personName != '') // required: anything besides N/A
    }
  }

  _validationErrors(val) {
    const errMsgs = {
      personNameValMsg: val.PersonVal ? '' : 'A person name is required'
    }
    return errMsgs;
  }
  ...
  render() {
    ...
    if (this.state.personName === undefined || this.state.personName ) {
          ...
          <form id="Form" className="form-horizontal">
            <div className="form-group">
              <label className="col-md-12 control-label">
                <h1 style={{textAlign:'center'}}>Step 1: Welcome to person detail 1</h1>
              </label><br/>
